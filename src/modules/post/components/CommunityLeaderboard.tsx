...
import { getPostCommunity } from "@/modules/post/api/api";
import { PostCommunity } from "@/modules/post/types/post";
...
export default function CommunityLeaderboard({
  ...
  const [data, setData] = useState<PostCommunity | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchCommunityData = async () => {
      try {
        const result = await getPostCommunity(postId);
        setData(result);
      } catch (error) {
