...
export default function ContributorCard({ contribution }: ContributorCardProps) {
    const { post, total_amount, updated_at } = contribution;
    const progress = Math.min(100, Math.round((post.current_amount / post.goal_amount) * 100));
                    ...
                    <CardContent sx={{ flex: 1, p: 2, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                        <Stack spacing={1.5}>
                            <Box display="flex" justifyContent="space-between" alignItems="flex-start">
                                <Typography variant="h6" fontWeight={700} sx={{ lineClamp: 1, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 1, WebkitBoxOrient: "vertical" }}>
                                    {post.post_header}
                                </Typography>
                                <Chip
                            ...
                            <Typography variant="body2" color="text.secondary" sx={{ lineClamp: 2, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", minHeight: 40 }}>
